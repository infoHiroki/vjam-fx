...
- Vitest + jsdom (testing, 1775 tests)

## Architecture
- **Popup**: `popup/` — UI controller, injects via `chrome.scripting.executeScript`
- **Content (MAIN world)**: `content/` — VJamFXEngine, 204 presets (15カテゴリ), video audio capture
...
- **Toast**: `_showToast(msg)` — 1.5秒表示、Reset/Next/Scene/Text操作時に表示
...
## Testing
```bash
npm test          # vitest run (1775 tests)
npm run test:watch
```
