...
describe('VJamFXEngine', () => {
  ...
  describe('handleMessage validation', () => {
    ...
    it('should ignore message without string action', () => {
    ...
    it('should ignore startAutoCycle with non-array presets', () => {
      engine.handleMessage({ action: 'startAutoCycle', presets: 'not-array' });
      // Should not throw, auto-cycle should not start
    });
    ...
    it('should ignore textAutoStart with non-string text', () => {
      engine.handleMessage({ action: 'textAutoStart', text: 123 });
      // Should not throw
    });
  });
  ...
  describe('handleBatch', () => {
    it('should process multiple messages in order', () => {
      const spy = vi.spyOn(engine, 'handleMessage');
      engine.handleBatch([
        { action: 'setBlendMode', blendMode: 'difference' },
        { action: 'setOpacity', opacity: 0.5 },
      ]);
      expect(spy).toHaveBeenCalledTimes(2);
      expect(spy).toHaveBeenCalledWith({ action: 'setBlendMode', blendMode: 'difference' });
      expect(spy).toHaveBeenCalledWith({ action: 'setOpacity', opacity: 0.5 });
      spy.mockRestore();
    });
    ...
    it('should handle non-array input', () => {
      expect(() => engine.handleBatch('not-array')).not.toThrow();
  ...
  describe('stop() OSD cleanup', () => {
    it('should clear OSD timer on stop', () => {
      engine._osdTimer = setTimeout(() => {}, 10000);
      engine.stop();
