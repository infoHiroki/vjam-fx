...
describe('VJamFXEngine', () => {
  ...
  describe('stop() OSD cleanup', () => {
    it('should clear OSD timer on stop', () => {
      engine.startPreset('neon-tunnel');
      engine.showOSD('test');
      expect(engine._osdTimer).not.toBeNull();
      engine.stop();
      expect(engine._osdTimer).toBeNull();
    });

    it('should remove OSD element on stop', () => {
      engine.startPreset('neon-tunnel');
      engine.showOSD('test');
      expect(engine._osdEl).not.toBeNull();
      engine.stop();
  ...
  describe('_removeLayer detached container', () => {
    it('should handle detached container without throwing', () => {
      engine.startPreset('neon-tunnel');
      const layer = engine.activeLayers.get('neon-tunnel');
      // Detach container from DOM
      if (layer) layer.container.remove();
      expect(() => engine._removeLayer('neon-tunnel')).not.toThrow();
      expect(engine.activeLayers.has('neon-tunnel')).toBe(false);
    });
  });
  ...
  describe('handleMessage validation', () => {
    ...
    it('should ignore message without action string', () => {
      expect(() => engine.handleMessage({ action: 123 })).not.toThrow();
      expect(() => engine.handleMessage({})).not.toThrow();
    });

    it('should ignore addLayer with non-string preset', () => {
      engine.createOverlay();
      engine.handleMessage({ action: 'addLayer', preset: 123 });
      expect(engine.activeLayers.size).toBe(0);
    });

    it('should ignore removeLayer with non-string preset', () => {
      expect(() => engine.handleMessage({ action: 'removeLayer', preset: null })).not.toThrow();
    });

    it('should ignore toggleLayer with non-string preset', () => {
      expect(() => engine.handleMessage({ action: 'toggleLayer', preset: undefined })).not.toThrow();
    });

    it('should ignore textDisplay with text over 200 chars', () => {
      engine.createOverlay();
      const longText = 'a'.repeat(201);
      engine.handleMessage({ action: 'textDisplay', text: longText });
      // Should not crash
    });

    it('should accept textDisplay with text under 200 chars', () => {
      engine.createOverlay();
      expect(() => engine.handleMessage({ action: 'textDisplay', text: 'hello' })).not.toThrow();
    });

    it('should ignore textAutoStart with non-string text', () => {
      expect(() => engine.handleMessage({ action: 'textAutoStart', text: 123 })).not.toThrow();
    });

    it('should ignore startAutoCycle with non-array presets', () => {
      expect(() => engine.handleMessage({ action: 'startAutoCycle', presets: 'not-array' })).not.toThrow();
    });
  });
  ...
  describe('handleBatch', () => {
    ...
    it('should handle empty array', () => {
      expect(() => engine.handleBatch([])).not.toThrow();
    });

    it('should handle non-array input', () => {
      expect(() => engine.handleBatch(null)).not.toThrow();
      expect(() => engine.handleBatch('foo')).not.toThrow();
    });
  });

  describe('beforeunload listener', () => {
    it('should register beforeunload listener in _ensureListeners', () => {
      expect(engine._onBeforeUnload).toBeDefined();
      expect(typeof engine._onBeforeUnload).toBe('function');
    });

    it('should remove beforeunload listener on destroy', () => {
      engine.destroy();
      expect(engine._onBeforeUnload).toBeNull();
    });
  });
});
