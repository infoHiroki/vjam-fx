/**
 * VJam FX — Content Script Engine (iOS / Safari Web Extension)
 * Modified from content.js for iOS environment.
 * No bridge listener, no tabCapture — uses mic audio or MSE interceptor as fallback.
 ...
 * - Audio: createMediaElementSource → MSE interceptor → mic fallback
 */
(function() {
  'use strict';
  ...
  class VJamFXEngine {
    constructor() {
      ...
      // Mic audio capture (iOS fallback)
      this._micAudioCtx = null;
      this._micAudioStream = null;
    ...
    /**
     * Register fullscreen listener if not already present.
    ...
    _ensureListeners() {
      if (!this._onFullscreenChange) {
        this._onFullscreenChange = () => {
          ...
          var targetParent = fsEl || document.body;
    ...
    _startMicAudio() {
      ...
      navigator.mediaDevices.getUserMedia({ audio: true }).then(function(stream) {
        ...
        self._micAudioCtx = ctx;
        self._micAudioStream = stream;
        self._videoAudioAnalyser = analyserNode;
        var binCount = analyserNode.frequencyBinCount;
        var freqPerBin = ctx.sampleRate / analyserNode.fftSize;
        self._videoAudioFreqData = new Float32Array(binCount);
        self._videoAudioTimeData = new Float32Array(analyserNode.fftSize);
        self._videoAudioBassLow = Math.min(Math.floor(20 / freqPerBin), binCount);
        self._videoAudioBassHigh = Math.min(Math.floor(250 / freqPerBin), binCount);
        self._videoAudioMidHigh = Math.min(Math.floor(4000 / freqPerBin), binCount);
        self._videoAudioTrebleHigh = Math.min(Math.floor(16000 / freqPerBin), binCount);
        self._videoAudioBassMax = 1e-6;
        self._videoAudioMidMax = 1e-6;
        self._videoAudioTrebleMax = 1e-6;
        self._videoAudioRmsHistory = [];
        self._videoAudioLastBeatTime = -1;
        self._videoAudioOnsetTimes = [];
        self._videoAudioTempo = 120;
      }).catch(function(e) {
        console.warn('VJam FX: mic access denied', e);
      });
    }

    _stopMicAudio() {
      if (this._micAudioStream) {
        this._micAudioStream.getTracks().forEach(function(t) { t.stop(); });
        this._micAudioStream = null;
      }
      if (this._micAudioCtx && this._micAudioCtx.state !== 'closed') {
        this._micAudioCtx.close().catch(function() {});
      }
      this._micAudioCtx = null;
    }
    ...
    _connectMediaElement(media) {
      ...
      try {
        ...
        this._videoAudioOnsetTimes = [];
        this._videoAudioTempo = 120;
        // Delay — verify analyser produces non-silent data first
        ...
        var startSilenceCheck = function() {
          ...
          var checkTimer = self._silenceCheckTimer = setInterval(function() {
            ...
            if (hasSignal) {
              // Real audio confirmed from media element
              console.log('VJam FX: media element audio confirmed');
              clearInterval(checkTimer);
            } else if (checkCount >= 10) {
              // 2 seconds of silence — CORS/MSE restriction likely
              // Disconnect our silent analyser
              ...
              clearInterval(checkTimer);
              // Check for MSE interceptor analyser
              if (window._vjamMSEAnalyser) {
                self._videoAudioAnalyser = window._vjamMSEAnalyser;
                var mseAnalyser = window._vjamMSEAnalyser;
                var mseBinCount = mseAnalyser.frequencyBinCount;
                var mseFreqPerBin = (mseAnalyser.context ? mseAnalyser.context.sampleRate : 44100) / mseAnalyser.fftSize;
                self._videoAudioFreqData = new Float32Array(mseBinCount);
                self._videoAudioTimeData = new Float32Array(mseAnalyser.fftSize);
                self._videoAudioBassLow = Math.min(Math.floor(20 / mseFreqPerBin), mseBinCount);
                self._videoAudioBassHigh = Math.min(Math.floor(250 / mseFreqPerBin), mseBinCount);
                self._videoAudioMidHigh = Math.min(Math.floor(4000 / mseFreqPerBin), mseBinCount);
                self._videoAudioTrebleHigh = Math.min(Math.floor(16000 / mseFreqPerBin), mseBinCount);
                self._videoAudioBassMax = 1e-6;
                self._videoAudioMidMax = 1e-6;
                self._videoAudioTrebleMax = 1e-6;
                self._videoAudioRmsHistory = [];
                ...
                self._videoAudioOnsetTimes = [];
                self._videoAudioTempo = 120;
                console.log('VJam FX: using MSE interceptor analyser');
              } else {
                // Fall back to mic
                self._startMicAudio();
              }
        ...
        // Fall back to MSE interceptor or mic
        if (ctx && ctx.state !== 'closed') ctx.close().catch(function() {});
        if (window._vjamMSEAnalyser) {
          this._videoAudioAnalyser = window._vjamMSEAnalyser;
          var mseAnalyser = window._vjamMSEAnalyser;
          var mseBinCount = mseAnalyser.frequencyBinCount;
          var mseFreqPerBin = (mseAnalyser.context ? mseAnalyser.context.sampleRate : 44100) / mseAnalyser.fftSize;
          this._videoAudioFreqData = new Float32Array(mseBinCount);
          this._videoAudioTimeData = new Float32Array(mseAnalyser.fftSize);
          this._videoAudioBassLow = Math.min(Math.floor(20 / mseFreqPerBin), mseBinCount);
          this._videoAudioBassHigh = Math.min(Math.floor(250 / mseFreqPerBin), mseBinCount);
          this._videoAudioMidHigh = Math.min(Math.floor(4000 / mseFreqPerBin), mseBinCount);
          this._videoAudioTrebleHigh = Math.min(Math.floor(16000 / mseFreqPerBin), mseBinCount);
        } else {
          this._startMicAudio();
        }
      }
